...
import static com.daogukeji.dapeng.fragment.BuGuang.BUGUANG_CLOSE;
...
public class BuGuangIntentService_close extends IntentService{
    public static final String TAG = "BuGuangIntentService_close";
    ...
    public BuGuangIntentService_close(){
        super("BuGuangIntentService_close");
    }
    @Override
        ...
        Thread recThread = new Thread(){
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（补光继电器返回数据的长度为5）
                        byte[] buf_data = new byte[size];
                        for (int i = 0; i < size; i++) {
                            buf_data[i] = buffer[i];
                        }
                        String buguang = new String(byte2hex(buf_data));//将返回的数据转为String
                        //Log.i(TAG, "send_rec: "+buguang);
                        intent.setAction(BUGUANG_CLOSE);//设置广播标志
                        if("8083100001".equals(buguang)){//排风开启成功返回的数据
                            intent.putExtra("buguang_close","补光已关闭...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("buguang_close","补光关闭失败，请重试");
