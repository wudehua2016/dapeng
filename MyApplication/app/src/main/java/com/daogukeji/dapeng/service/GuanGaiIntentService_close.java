...
public class GuanGaiIntentService_close extends IntentService{
    public static final String TAG = "GuanGaiIntentService_close";
    ...
    public GuanGaiIntentService_close(){
        super("GuanGaiIntentService_close");
    }
    @Override
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（灌溉继电器返回数据的长度为5）
                        ...
                        intent.setAction(GUANGAI_CLOSE);//设置广播标志
                        if("8083100001".equals(guangai)){//排风开启成功返回的数据
                            intent.putExtra("guangai_close","灌溉已关闭...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("guangai_close","灌溉关闭失败，请重试");
