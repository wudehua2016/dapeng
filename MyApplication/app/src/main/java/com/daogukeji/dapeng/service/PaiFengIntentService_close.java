...
public class PaiFengIntentService_close extends IntentService{
    public static final String TAG = "PaiFengIntentService_close";
    ...
    public PaiFengIntentService_close(){
        super("PaiFengIntentService_close");
    }
    @Override
        ...
        Thread recThread = new Thread(){
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（排风继电器返回数据的长度为5）
                        byte[] buf_data = new byte[size];
                        for (int i = 0; i < size; i++) {
                            buf_data[i] = buffer[i];
                        }
                        String paifeng = new String(byte2hex(buf_data));//将返回的数据转为String
                        Log.i(TAG, "send_rec: "+paifeng);
                        intent.setAction(PAIFENG_CLOSE);//设置广播标志
                        if("8083100001".equals(paifeng)){//排风开启成功返回的数据
                            intent.putExtra("paifeng_close","排风已关闭...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("paifeng_close","排风关闭失败，请重试");
