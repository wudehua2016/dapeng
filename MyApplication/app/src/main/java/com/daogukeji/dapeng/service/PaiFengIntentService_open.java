...
public class PaiFengIntentService_open extends IntentService{
    public static final String TAG = "PaiFengIntentService_open";
    ...
    public PaiFengIntentService_open(){
        super("PaiFengIntentService_open");
    }
    @Override
        ...
        Thread recThread = new Thread(){
            ...
            public void run(){
                ...
                try{
                    ...
                    System.out.println("排风数据长度》》》》》》》》》》》》"+size);
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（排风继电器返回数据的长度为5）
                        ...
                        intent.setAction(PAIFENG_OPEN);//设置广播标志
                        if("8083100001".equals(paifeng)){//排风开启成功返回的数据
                            intent.putExtra("paifeng_open","排风已开启...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("paifeng_open","排风开启失败，请重试");
