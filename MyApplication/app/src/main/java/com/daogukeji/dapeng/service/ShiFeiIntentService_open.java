...
import static com.daogukeji.dapeng.fragment.ChuShi.CHUSHI_OPEN;
import static com.daogukeji.dapeng.fragment.PaiFeng.PAIFENG_OPEN;
import static com.daogukeji.dapeng.fragment.ShiFei.SHIFEI_OPEN;

/**
 * Created by dell on 2017/6/22.
 */

public class ShiFeiIntentService_open extends IntentService{
    public static final String TAG = "ShiFeiIntentService_open";
    ...
    public ShiFeiIntentService_open(){
        super("ShiFeiIntentService_open");
    }
    @Override
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（施肥继电器返回数据的长度为5）
                        ...
                        intent.setAction(SHIFEI_OPEN);//设置广播标志
                        if("8083100001".equals(shifei)){//排风开启成功返回的数据
                            intent.putExtra("shifei_open","施肥已开启...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("shifei_open","施肥开启失败，请重试");
