...
public class ChuShiIntentService_open extends IntentService{
    public static final String TAG = "ChuShiIntentService_open";
    ...
    public ChuShiIntentService_open(){
        super("ChuShiIntentService_open");
    }
    @Override
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（除湿继电器返回数据的长度为5）
                        ...
                        intent.setAction(CHUSHI_OPEN);//设置广播标志
                        if("8083100001".equals(chushi)){//排风开启成功返回的数据
                            intent.putExtra("chushi_open","除湿已开启...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("chushi_open","除湿开启失败，请重试");
