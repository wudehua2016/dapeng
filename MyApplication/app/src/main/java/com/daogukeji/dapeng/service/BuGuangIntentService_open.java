...
import static com.daogukeji.dapeng.fragment.BuGuang.BUGUANG_OPEN;
...
public class BuGuangIntentService_open extends IntentService{
    public static final String TAG = "BuGuangIntentService_open";
    ...
    public BuGuangIntentService_open(){
        super("BuGuangIntentService_open");
    }
    @Override
            ...
            public void run(){
                ...
                try{
                    ...
                    if(size==5){//此判断用于辨别串口返回数据内容，防止接到别的指令返回的数据（补光继电器返回数据的长度为5）
                        ...
                        intent.setAction(BUGUANG_OPEN);//设置广播标志
                        if("8083100001".equals(buguang)){//排风开启成功返回的数据
                            intent.putExtra("buguang_open","补光已开启...");//添加广播数据
                            sendBroadcast(intent);//发送广播
                        }else{
                            intent.putExtra("buguang_open","补光开启失败，请重试");
